...
class AnWPFL_Competition extends CPT_Core {
	...
	public function get_competition_list( $args ) {
		...
		if ( ! empty( $args->league_ids ) ) {
			$tax_query[] = [
				'taxonomy' => 'anwp_league',
				'terms'    => wp_parse_id_list( $args->league_ids ),
			];
		}

		if ( ! empty( $args->season_ids ) ) {
			$tax_query[] = [
				'taxonomy' => 'anwp_season',
				'terms'    => wp_parse_id_list( $args->season_ids ),
			];
		}
		...
		$competitions_posts = get_posts( $post_args );

		if ( empty( $competitions_posts ) ) {
			return [];
		}

		$competitions = [];

		/** @var WP_Post $competitions_post */
		foreach ( $competitions_posts as $competitions_post ) {

			// Get Season and League
			$terms   = wp_get_post_terms( $competitions_post->ID, [ 'anwp_league', 'anwp_season' ] );
			$league  = [];
			$seasons = [];
			...
			if ( is_array( $terms ) ) {
				...
				foreach ( $terms as $term ) {
					...
					if ( 'anwp_league' === $term->taxonomy && $term->term_id ) {
						$league['title']   = $term->name;
						$league['country'] = get_term_meta( $term->term_id, '_anwpfl_country', true );
					}
			...
			$logo = anwp_football_leagues()->helper->string_to_bool( $args->show_logo ) ? get_post_meta( $competitions_post->ID, '_anwpfl_logo', true ) : '';

			if ( empty( $league ) || empty( $seasons ) ) {
				continue;
			}

			$competitions[] = [
				'id'      => $competitions_post->ID,
				'link'    => get_permalink( $competitions_post ),
				'title'   => $competitions_post->post_title,

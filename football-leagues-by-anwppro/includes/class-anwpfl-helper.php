...
class AnWPFL_Helper {
	...
	public function create_new_league( WP_REST_Request $request ) {
		...
		return $insert_result
			? rest_ensure_response( [ 'leagues' => anwp_football_leagues()->league->get_leagues_list() ] )
			: rest_ensure_response( new WP_Error( 'rest_invalid', esc_html__( 'Saving Data Error', 'anwp-football-leagues' ), [ 'status' => 400 ] ) );
	...
	public function create_new_season( WP_REST_Request $request ) {
		...
		return $insert_result
			? rest_ensure_response( [ 'seasons' => anwp_football_leagues()->season->get_seasons_list() ] )
			: rest_ensure_response( new WP_Error( 'rest_invalid', esc_html__( 'Saving Data Error', 'anwp-football-leagues' ), [ 'status' => 400 ] ) );
	...
	public function create_metabox_header( $data ) {
		...
		// @formatter:off
		?>
		<div class="anwp-border anwp-border-gray-500 <?php echo esc_attr( $data['classes'] ); ?>" id="<?php echo esc_attr( $data['slug'] ); ?>">
